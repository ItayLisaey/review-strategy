...
// Get dependency relationships from the graph
export function getDependencyRelationships(
  files: FileChange[],
  fileContents: Map<string, string>
  ...
  graph.edges().forEach((edge) => {
    edges.push({ from: edge.v, to: edge.w });
  });
...
export async function analyzeWithDependencies(
  ...
  try {
    ...
    const sortedFiles = [...files].sort((a, b) => {
      ...
      const aIsConfig =
        a.filename.includes("config") ||
        a.filename.endsWith(".json") ||
        a.filename.endsWith(".yml");
      const bIsConfig =
        b.filename.includes("config") ||
        b.filename.endsWith(".json") ||
        b.filename.endsWith(".yml");
      if (aIsConfig && !bIsConfig) return -1;
      if (!aIsConfig && bIsConfig) return 1;

      // 2. Type definitions
      const aIsTypes =
        a.filename.includes(".d.ts") || a.filename.includes("types");
      const bIsTypes =
        b.filename.includes(".d.ts") || b.filename.includes("types");
      ...
      // 5. Tests last
      const aIsTest =
        a.filename.includes("test") || a.filename.includes("spec");
      const bIsTest =
        b.filename.includes("test") || b.filename.includes("spec");
    ...
    if (
      ...
      reviewFlags.push({
        ...
        description:
          "Verify configuration changes won't break existing functionality",
      });
    }
